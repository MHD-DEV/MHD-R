...
REAL SOMALOBS !SOMATORIO DOS LOGARITMOS DE VAZÃO OBSERVADOS
REAL SOMALCAL !SOMATORIO DOS LOGARITMOS DE VAZÃO CALCULADOS
...
REAL XMOBS,XMLOBS !VAZÃO MÉDIA OBSERVADA E VAZAO MEDIA DOS LOGARITMOS DAS OBSERVACOES
REAL XMCAL,XMLCAL !VAZÃO MÉDIA CALCULADA E VAZAO MEDIA DOS LOGARITMOS DAS VAZOES CALCULADAS
!REAL NASH(NB),LNASH(NB),R2(NB),ERRV(NB),ERRE(NB),R2L(NB) !COEF. NASH, ERRO NOS VOLUMES, COEF NASH LOGARITMOS
REAL ETCAL(NB),ETOBS(NB)
...
!				SOMAECAL=SOMAECAL+ETTB(IB,IT) ! SOMA EVAPORACAO TOTAL CALCULADA DA BACIA
        ...
        IF(ITVAL > 25) THEN
			...
			DO IT=INIAJU,NT
				...
				ENDIF					
			ENDDO
			SOMAEOBS=(SOMAPREB-(SOMAOBS+QCONS(IB)*ITVAL)*(DTP/1000.)/ADREN(IQOBS(IB))) ! SOMA EVAPORACAO MEDIA OBSERVADA
			...
			ERRE(IB)=(SOMAECAL-SOMAEOBS)/SOMAEOBS
			ETCAL(IB)=SOMAECAL/ITVAL*(86400/DTP)
			ETOBS(IB)=SOMAEOBS/ITVAL*(86400/DTP)
