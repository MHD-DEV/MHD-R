subroutine balanco
use vars_main, only: nb,nc,it,nt,iano,jdia,ihoraini,dtp,dir_dados,icdom,ncdom,qr,ihoraini, &
& idini,qobs,adren,iexut,imes,ibac,kcb !,preb,taall,tdewall,vvall,patmall,rocall
!use vars_main, only: nb,nc,it,nt,iano,jdia,ihoraini,dtp,dir_dados,preall,icdom,ncdom,qr,iihoraini, &
!& idini,qobs,adren,iexut,ettb,imes,fmes,ibac,kcb,preb,taall,tdewall,vvall,patmall,rocall
implicit none
...
!    ettsimmedia = 0.
    ...
    if(imes==1) incrementoano=ianoaux
    itiniah = julday(imes,1,ianoaux) !converte o dia de inicio em calendario juliano                   
    itfimah = julday(fmes,diasmes(fmes),incrementoano) !converte o dia de inicio em calendario juliano
...
!                ettsimmedia = ettsimmedia/real(ncdom)/real(ntaux)
                ...
                write(*,'(i12,10f12.2)')ianoaux,qobsmedia,tamedia,tdewmedia,vvmedia,patmmedia,&
                                                & rocmedia,precmedia,qsimmedia,ettsimmedia,erro
                ianoaux = ianoaux + 1
                erro=0.
                tamedia = 0.
