...
!	write(*,*) IMES,IDIA,IANO,IMES1,IDIA1,IANO1,IHORAINI,IHORA1,IHORAINI-IHORA1
    NSTEPC=(JULDAY(IMES,IDIA,IANO,366)-JULDAY(IMES1,IDIA1,IANO1,366))*86400+(IHORAINI-IHORA1)*3600 
!	WRITE(*,*) NSTEPC  
	NSTEPC=NSTEPC/INT(DTP)
!	WRITE(*,*) NSTEPC  
	IF(NSTEPC < 0) THEN ! OS DADOS METEOROLOGICOS SE INICIAM APOS O PERIODO DE SIMULACAO
	...
	DO IT=1,NT/8+1
		READ(FILOBS,'(A<IAUX>,1x,<NOBS>G12.2)') TITULO(1:IAUX),(QOBS(K,IT),K=1,NOBS)
!		WRITE(*,*)  TITULO(1:IAUX),(QOBS(K,IT),K=1,NOBS)
	ENDDO
	WRITE(*,*)
	WRITE(*,'(A20,/,A<IAUX>,<NOBS>F12.2)') ' ULTIMA VAZAO LIDA  ',TITULO(1:IAUX),(QOBS(K,NT/8+1),K=1,NOBS)
	WRITE(*,*)
	CLOSE (FILOBS)
71	FORMAT(A80)
